...
/**
 * GET /api/pertes/[id]
 * Returns detailed info for a specific perte declaration
 */
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  const { data: perte, error } = await admin
  ...
  const [{ data: ownedBoutique }, { data: assignedEmploye }] = await Promise.all([
    ...
    admin
      .from('employes')
      .select('id')
      .eq('boutique_id', perte.boutique_id)
      .eq('proprietaire_id', user.id)
      .eq('actif', true)
      .maybeSingle(),
  ]);
  ...
  return NextResponse.json({ data: perte as PerteDeclaration });
}
