  ...
  return (
    <div style={{ ...cardStyle(80), background: 'var(--sp-ink)', borderColor: 'transparent', color: 'var(--sp-cream)' }}>
      ...
      <Link
        ...
        style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 8, background: 'var(--sp-sky)', color: '#fff', fontSize: 12, fontWeight: 600, letterSpacing: '.04em', textDecoration: 'none' }}
      >
        Voir les rapports →
      </Link>
    </div>
...
function RecentOrders({ transactions, boutiques }: { transactions: RecentTx[]; boutiques: Boutique[] }) {
...
export default function DashboardHome({
  ...
  const prev7Start = (() => { const d = new Date(); d.setDate(d.getDate() - 13); return d.toISOString().split('T')[0]; })();
  const prev7End = (() => { const d = new Date(); d.setDate(d.getDate() - 7); return d.toISOString().split('T')[0]; })();

  const last7Total = (key: 'ca' | 'transactions') =>
    weeklyStats.filter((s) => last7Days.includes(s.date)).reduce((sum, s) => sum + s[key], 0);

  const prev7Total = (key: 'ca' | 'transactions') =>
    weeklyStats.filter((s) => s.date >= prev7Start && s.date <= prev7End).reduce((sum, s) => sum + s[key], 0);
  ...
  const totalBenefice = weeklyStats.filter((s) => last7Days.includes(s.date)).reduce((sum, s) => sum + s.benefice, 0);
  const now = new Date();
