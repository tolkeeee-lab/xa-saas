...
export default function MobileNav() {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-xa-surface border-t border-xa-border z-40">
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex flex-col items-center justify-center py-2.5 px-3 text-xs transition-colors shrink-0 min-w-[64px] ${
                isActive ? 'text-xa-primary' : 'text-xa-muted'
              }`}
            >
              {item.icon}
