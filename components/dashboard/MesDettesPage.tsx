...
export default function MesDettesPage({ data: initialData }: MesDettesPageProps) {
  ...
  async function handleDetteSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!res.ok) {
      ...
      setDetteFormError((err as { error?: string }).error ?? 'Erreur lors de la création.');
      return;
    }
  ...
  async function handleDeleteDette(id: string) {
    ...
    if (res.ok) {
      ...
      const err = await res.json();
      showToast((err as { error?: string }).error ?? 'Erreur lors de la suppression.', 'error');
    }
  }
  ...
  async function handleMarkRembourse(id: string) {
    ...
    if (res.ok) {
      ...
      const err = await res.json();
      showToast((err as { error?: string }).error ?? 'Erreur.', 'error');
    }
  }
  ...
  async function handleRembSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (res.ok) {
      ...
      const err = await res.json();
      setRembError((err as { error?: string }).error ?? 'Erreur.');
    }
  }

  const nbActives = dettes.filter(
    (d) => d.statut === 'en_cours' || d.statut === 'en_retard',
  ).length;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-xl font-bold text-xa-text">Mes dettes</h1>
          <p className="text-sm text-xa-muted mt-0.5">
            Suivez vos dettes personnelles et vos remboursements
          </p>
        </div>
        <button
      ...
      {dettes.length === 0 ? (
        ...
        <div className="bg-xa-surface border border-xa-border rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-xa-border">
            <h2 className="font-semibold text-xa-text text-sm">Liste des dettes</h2>
          </div>
                ...
                {dettes.map((d) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-xa-text font-semibold">
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`text-xs font-semibold px-2 py-0.5 rounded-full ${STATUT_COLORS[d.statut]}`}
                        >
              ...
              {totals.total_en_cours > 0 && (
                <tfoot>
                  <tr className="border-t border-xa-border bg-xa-bg">
                    <td
                      colSpan={4}
                      className="px-4 py-3 text-xs font-semibold text-xa-muted uppercase tracking-wider text-right"
                    >
            ...
            <form onSubmit={handleDetteSubmit} className="p-5 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setDetteForm((f) => ({ ...f, montant_rembourse: e.target.value }))
                  }
                  className="w-full px-3 py-2 rounded-lg border border-xa-border bg-xa-bg text-xa-text text-sm focus:outline-none focus:ring-2 focus:ring-xa-primary"
                />
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-xa-text mb-1">Notes</label>
                <textarea
          ...
          <div className="bg-xa-surface rounded-xl shadow-xl w-full max-w-sm">
            ...
            <div className="px-5 pt-4">
              <p className="text-sm text-xa-muted">
                Dette :{' '}
                <span className="font-semibold text-xa-text">{rembDette.libelle}</span>
              </p>
