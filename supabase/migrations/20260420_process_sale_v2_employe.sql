-- Migration: extend process_sale_v2 to accept p_employe_id
-- (transactions.employe_id column already exists from prior migration)
--
-- This is idempotent via CREATE OR REPLACE.
-- All existing behaviour is preserved; p_employe_id defaults to NULL so
-- existing callers that omit it continue to work unchanged.
...
CREATE OR REPLACE FUNCTION process_sale_v2(
  ...
  p_employe_id        uuid     DEFAULT NULL   -- NEW: employee who performed the sale
)
RETURNS jsonb
LANGUAGE plpgsql
AS $$
...
BEGIN
  ...
  -- Insert transaction (includes employe_id)
...
GRANT EXECUTE ON FUNCTION process_sale_v2 TO authenticated, anon, service_role;
